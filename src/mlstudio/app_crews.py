"""app_crews.py - Angie Crews custom example.

An example of a supervised regression workflow customized for this project.
...
def predict_example(model: LinearRegression) -> None:
    ...
    LOG.info("Predicting one new example")
...
def make_plots(df_clean: pd.DataFrame, model: LinearRegression) -> None:
    """Create charts for the supervised regression workflow."""
...
def main() -> None:
    """Main function to run the customized supervised ML workflow."""
    log_header(LOG, "ML")
